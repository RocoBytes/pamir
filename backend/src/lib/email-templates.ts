...
function clausulaCheck(label: string, accepted: boolean): string {
  const icon = accepted ? '&#10003;' : '&#10007;';
  const color = accepted ? GREEN : '#dc2626';
  return `<span style="display:inline-block;margin-right:16px;color:${color};font-size:13px;">${icon} ${label}</span>`;
}
        ...
        <tr>
          <td style="padding:0 32px 24px;">
            <table width="100%" cellpadding="0" cellspacing="0" style="border:1px solid ${BORDER};border-radius:6px;overflow:hidden;border-collapse:collapse;">
              ...
              ${sectionHeader('IV. Cláusulas aceptadas')}
              <tr>
                <td colspan="2" style="padding:12px 12px;font-size:13px;color:#1f2937;border-bottom:1px solid ${BORDER};">
                  ${clausulaCheck('Declaración de salud', data.declaracionSalud)}
                  ${clausulaCheck('Aceptación de riesgo', data.aceptacionRiesgo)}
                  ${clausulaCheck('Consentimiento de datos', data.consentimientoDatos)}
                  ${clausulaCheck('Derecho de imagen', data.derechoImagen)}
                </td>
              </tr>
        ...
        <tr>
          <td style="background:#f9fafb;padding:20px 32px;border-top:1px solid ${BORDER};">
            <p style="margin:0;color:${GRAY};font-size:12px;line-height:1.6;">
              Este correo es un comprobante automático de tu registro.<br>
              Si no realizaste este registro o tienes dudas, por favor ignora este mensaje.
            </p>
          </td>
        </tr>
